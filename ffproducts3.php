...
#one{background-image: url(img/Women_Flats_3b.jpg);background-size: cover; }

#two{background-image: url(img/Women_Flats_3c.jpg); background-size: cover;}

#three{background-image: url(img/Women_Flats_3d.jpg);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Women_Flats_3b.jpg) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Women_Flats_3c.jpg);
}

#three:checked ~ .display-img{
    background-image: url(img/Women_Flats_3d.jpg);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=27");
            $stmt -> execute();
    ...
    <div class="text3">
        <ul>
        <li><h4>Sole: fabric</h4></li>
        <li><h4>Closure: Buckle</h4></li>
        <li><h4> Shoe Width: Medium</h4></li>
        <li><h4> Sole Material: TPC Which is high in durability, Anti Slip Line in sole for slip free movement</h4></li>
        <li><h4>EVA used for coushioning between sole and footbed for better comfort</h4></li>
        <li><h4> Bow styling detail on upper, Back Ankle Strap with Buckle Closure</h4></li>
        <li><h4> Premium Quality synthetic used</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=25");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=26");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=28");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
