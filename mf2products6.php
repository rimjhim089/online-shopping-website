...
#one{background-image: url(img/Men_CasualShoes_2b.jpg);background-size: cover; }

#two{background-image: url(img/Men_CasualShoes_2c.jpg); background-size: cover;}

#three{background-image: url(img/Men_CasualShoes_2d.jpg);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Men_CasualShoes_2b.jpg) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Men_CasualShoes_2c.jpg);
}

#three:checked ~ .display-img{
    background-image: url(img/Men_CasualShoes_2d.jpg);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=6");
            $stmt -> execute();
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=5");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=7");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=8");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
