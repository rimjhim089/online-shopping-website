...
#one{background-image: url(img/Men_TrackPants_3b.jpg);background-size: cover; }

#two{background-image: url(img/Men_TrackPants_3c.jpg); background-size: cover;}

#three{background-image: url(img/Men_TrackPants_3d.jpg);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Men_TrackPants_3b.jpg) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Men_TrackPants_3c.jpg);
}

#three:checked ~ .display-img{
    background-image: url(img/Men_TrackPants_3d.jpg);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=23");
            $stmt -> execute();
    ...
    <div class="text3">
        <ul>
        <li><h4> Fit Type: Slim Fit</h4></li>
        <li><h4> Lycra lower for men with two side pockets: this lycra dryfit pant comes with two zipper side pockets which keep all your essential items safe while you are on-the-go or performing various exercises at the gym</h4></li>
        <li><h4> Jogger Trackpants with Reflector Brand logo : To be in regular workouts at the gym or you want to step out for a coffee or dinner with friends in sporty style. These track pants for you with the brand logo will help you flaunt your unique style wherever you go.</h4></li>
        <li><h4> 100% Lycra Soft & confortable absorbent sports wear pants : this stylish gym wear pant is absorbent and made of lycra which makes it stretchable and gives you comfort and ease of movements like GYM, CRICKET, RUNNING</h4></li>
        <li><h4> 100% Made in India Product with love</h4></li>
        <li><h4> Care Instructions: Don't use iron on Reflector Brand Logo and any sticker</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=24");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=22");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=21");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
