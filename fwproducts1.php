...
#one{background-image: url(img/Capture1.2.PNG);background-size: cover; }

#two{background-image: url(img/Capture1.3.PNG); background-size: cover;}

#three{background-image: url(img/Capture1.4.PNG);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Capture1.2.PNG) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Capture1.3.PNG);
}

#three:checked ~ .display-img{
    background-image: url(img/Capture1.4.PNG);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=33");
            $stmt -> execute();
    ...
    <div class="text3">
        <ul>
        <li><h4>Style:	Casual</h4></li>
        <li><h4>Color:	White</h4></li>
        <li><h4> Neckline:	Round Neck</h4></li>
        <li><h4> Length:	Regular</h4></li>
        <li><h4>Sleeve Length:	Short Sleeve</h4></li>
        <li><h4> Material:	Cotton</h4></li>
        <li><h4> Fabric:	Slight Stretch</h4></li>
        <li><h4>  Placket Type:	Pullovers</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=34");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=35");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=36");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
