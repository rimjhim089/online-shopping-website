...
#one{background-image: url(img/Women_Heels_3b.jpg);background-size: cover; }

#two{background-image: url(img/Women_Heels_3c.jpg); background-size: cover;}

#three{background-image: url(img/Women_Heels_3d.jpg);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Women_Heels_3b.jpg) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Women_Heels_3c.jpg);
}

#three:checked ~ .display-img{
    background-image: url(img/Women_Heels_3d.jpg);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=31");
            $stmt -> execute();
    ...
    <div class="text3">
        <ul>
        <li><h4>Closure: Zip</h4></li>
        <li><h4>Heel Height: 2.5 inches</h4></li>
        <li><h4> Material Type: Synthetic</h4></li>
        <li><h4> Heel Type: Cone heels</h4></li>
        <li><h4>Product warranty against manufacturing defects: 90 days</h4></li>
        <li><h4> Warranty Type: Manufacturer</h4></li>
        <li><h4> Care Instructions: Allow your pair of shoes to air and de-odorize at regular basis; use shoe bags to prevent any stains or mildew; dust any dry dirt from the surface using a clean cloth; do not use polish or shiner</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=29");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=30");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=32");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
