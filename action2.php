...
if(isset($_POST['pid'])){
    ...
    $stmt = $conn->prepare("select pro_title from buy where pro_title=?");
   ...
   if(!$code){
       $query = $conn->prepare("insert into buy(pro_id,pro_title,pro_keyword,pro_img,qty,total_price,pro_price,pro_click,uid) values(?,?,?,?,?,?,?,?,?)");
...
if(isset($_GET['remove'])){
    ...
    $stmt = $conn->prepare("delete from buy where pro_id=?");
 ...
 if(isset($_GET['clear'])){
    $stmt = $conn->prepare("delete from buy");
 ...
 if(isset($_POST['qty'])){
    ...
    $pprice = $_POST['pprice'];
     ...
     $stmt = $conn->prepare("update buy set qty=?,total_price=? where pro_id=?");
  ...
  if(isset($_POST['submit'])){
    ...
    $sql2="insert into display(pro_id,uid) select pro_id,uid from buy";
    $sql4="delete from buy where uid={$_SESSION['id']}"; 
