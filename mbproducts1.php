...
#one{background-image: url(img/Men_Jeans_1b.jpg);background-size: cover; }

#two{background-image: url(img/Men_Jeans_1c.jpg); background-size: cover;}

#three{background-image: url(img/Men_Jeans_1d.jpg);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Men_Jeans_1b.jpg) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Men_Jeans_1c.jpg);
}

#three:checked ~ .display-img{
    background-image: url(img/Men_Jeans_1d.jpg);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=17");
            $stmt -> execute();
    ...
    <div class="text3">
        ...
        <li><h4> Cotton & Elastane blend; Lycra Blend with Cotton for Superior Comfort.</h4></li>
        <li><h4> Mid Rise Stretchable; Comfort fit denim; Soothing pastel shades</h4></li>
        <li><h4> This pair of jeans from Studio Nexx will add instant style to your casual wardrobe and belongs to series of 'must have jeans'</h4></li>
        <li><h4> Zip fly with button closure & Brand label at the back. 2 Patch Pockets at Back, 1 Coin Pocket at Front, 2 Curved Pockets at Front</h4></li>
        <li><h4> Care Instructions: Machine Wash with like colors, Do Not Bleach, use mild detergent.</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=20");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=18");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=19");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
