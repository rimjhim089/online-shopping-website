...
#one{background-image: url(img/Jeans3.1.png);background-size: cover; }

#two{background-image: url(img/Jeans3.2.png); background-size: cover;}

#three{background-image: url(img/Jeans3.3.png);background-size: cover;}

#one:checked ~ .display-img{
    background-image: url(img/Jeans3.1.png) ;
}

#two:checked ~ .display-img{
    background-image: url(img/Jeans3.2.png);
}

#three:checked ~ .display-img{
    background-image: url(img/Jeans3.3.png);
}
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=39");
            $stmt -> execute();
    ...
    <div class="text3">
        ...
        <li><h4>Style: Vintage
Color: Navy</h4></li>
        <li><h4>Details: Button, Pocket, Zipper</h4></li>
        <li><h4> Pant Length: Long</h4></li>
        <li><h4>Composition: 85% Cotton, 13% Polyester, 2% Spandex</h4></li>
        <li><h4>Fabric: Slight Stretch</h4></li>
        <li><h4>Fit Type: Regular</h4></li>
        <li><h4>Waist Type: High Waist</h4></li>
        <li><h4>Closure Type: Button Fly, Zipper Flyy</h4></li>
        </ul>   
    </div>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=37");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=38");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=40");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
