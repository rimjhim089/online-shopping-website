...
#one{background-image: url(img/s2.jpg);background-size: cover; }
...
.fashion .shirt{
 ...
 <body>
     ...
     <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=42");
            $stmt -> execute();
    ...
    <div class="text3">
        <ul>
        <li><h4>Red-coloured and Yellow-coloured printed saree and has a zari border</h4></li>
        <li><h4>The model is wearing a stitched version of the blouse piece. The saree comes with an
unstitched blouse piece</h4></li>
        <li><h4> Saree fabric: linen blend</h4></li>
        <li><h4> Blouse fabric: linen blend</h4></li>
        <li><h4>Dry-clean</h4></li>
...
<div class="midcon">
        ...
        <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=41");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=43");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
   ...
   <?php
            include 'dbconn.php';
            $stmt = $conn->prepare("select * from products where pro_id=44");
            $stmt -> execute();
            $result = $stmt->get_result();
            while($row= $result->fetch_assoc()):
    ?>   
